...
<article <?php post_class( 'col-12 col-md-6 col-lg-4 masonry-item' ); ?> id="post-<?php the_ID(); ?>" data-groups='[<?php echo $data_groups; ?>]'>
	...
	<?php
		...
		if ( $category[1] ) {
			$category_link = '<a class="columns_hero_with_posts_post_footer_category" title="' . $category[1]->cat_name . '" href="' . get_category_link( $category[1]->term_id ) . '">' . $category[1]->cat_name . '</a>';
		}
